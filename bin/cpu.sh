...
if [ "$PLATFORM" == "Linux" ] ; then
	if [ "${ARCH}" == "x86_64" ] ; then
		if [ -x ~/envscripts/bin/tmux-mem-cpu-load.linux.x86_64 ] ; then
			tmux-mem-cpu-load.linux.x86_64 2
			exit
		fi
	fi
...
UPTIME=$(uptime | cut -d "," -f 3- | cut -d ":" -f2)

echo -n "${UPTIME}"

if [ -n "$cpu_user" ] && [ -n "$cpu_system" ] && [ -n "$cpu_idle" ]; then
    echo -n "${cpu_user}, ${cpu_system}, ${cpu_idle}" | awk -F', ' '{printf("%5.1f%%,%5.1f%%,%5.1f%%",$1,$2,$3)}'
fi
